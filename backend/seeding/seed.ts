...
console.log(
  `MONGODB_CLUSTER_NAME: ${process.env.MONGODB_CLUSTER_NAME || "NOT SET"}`
);
console.log(
  `MONGODB_CLUSTER_URL_ID: ${process.env.MONGODB_CLUSTER_URL_ID || "NOT SET"}`
); */
...
async function seedUserPreferences(
  ...
  for (const userPreferenceData of userPreferencesData) {
    ...
    if (
      userPreferenceData.latitude === undefined ||
      userPreferenceData.longitude === undefined
    ) {
      ...
      logging.warn(
        `Skipping user preference entry (missing latitude/longitude): ${
          userPreferenceData.id || userPreferenceData.email || "unknown"
        }`
      );
      continue;
    }
    ...
    if (userPreferenceData.id) {
      try {
        ...
        if (userPreference) {
          logging.info(
            `User preference with id ${userPreferenceData.id} already exists, reusing it`
          );
        } else {
        ...
        logging.warn(
          `Invalid id format for user preference ${
            userPreferenceData.id || userPreferenceData.email || "unknown"
          }, generating new ID`
        );
        ...
        userPreference = await UserPreferences.create(processedData);
        logging.info(
          `Created user preference with generated ID: ${userPreference._id}`
        );
      }
    } else {
...
async function seedCommunities(
  communitiesData: any[],
  userPreferencesMap: Map<string, any>,
  Community: any,
  logging: any
): Promise<Map<string, any>> {
    ...
    if (communityData.members) {
      processedData.members = communityData.members
        .map((identifier: string) => {
          return (
            (userPreferencesMap as any).byId?.get(identifier)?._id ||
            userPreferencesMap.get(identifier)?._id
          );
        })
        .filter(Boolean);
    }
  ...
  for (const messageData of messagesData) {
    ...
    if (messageData.authorId) {
      try {
        ...
        logging.warn(
          `Invalid authorId for message ${messageData.id || "unknown"}: ${
    ...
    if (!authorId) {
      logging.warn(
        `Author not found for message ${
          messageData.id || "unknown"
        }, skipping...`
      );
      continue;
    }
  ...
  for (const { message, parentMessageId } of messagesToUpdate) {
    if (parentMessageId) {
      ...
      if (parentMessage) {
        ...
        logging.info(
          `Updated message ${message._id} with parent ${parentMessage._id}`
        );
      } else {
        logging.warn(
          `Parent message ${parentMessageId} not found for message ${message._id}`
        );
      }
    }
  }
  ...
  for (const messageData of messagesData) {
    if (messageData.parentMessageId) {
      ...
      if (parentMessage && childMessage) {
        ...
        logging.info(
          `Added reply ${childMessage._id} to parent ${parentMessage._id}`
        );
      }
    }
  }
  ...
  for (const tipData of tipsData) {
    ...
    const processedData: any = {
      ...
      comments: (tipData.comments || [])
        .map((commentId: string) => messageMap.get(commentId)?._id)
        .filter(Boolean),
      likedBy: (tipData.likedBy || [])
        .map((identifier: string) => {
          return (
            (userPreferencesMap as any).byId?.get(identifier)?._id ||
            userPreferencesMap.get(identifier)?._id
    ...
    if (tipData.type === "pin") {
      ...
      if (tipData.startDate)
        processedData.startDate = new Date(tipData.startDate);
      if (tipData.duration !== undefined)
        processedData.duration = tipData.duration;
    }
...
async function seed() {
  ...
  const { UserPreferences, Message, Community, Tip } = await import(
    "../database/models"
  );
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      logging.error(
        "  - MONGODB_CLUSTER_URL_ID (if your cluster URL includes an ID)"
      );
    ...
    const userPreferencesMap = await seedUserPreferences(
      seedData.users || [],
      UserPreferences,
      logging
    );
    if ((userPreferencesMap as any).byId?.size === 0) {
      logging.info(
        "No valid UserPreferences entries found in users.json (requires latitude/longitude). Messages and tips that reference users will be skipped."
      );
    }

    let communityMap = new Map<string, any>();
    if (seedData.communities && seedData.communities.length > 0) {
      communityMap = await seedCommunities(
